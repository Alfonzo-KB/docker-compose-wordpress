...
function initYearsTab(data){
	...
	for (var i = 0; i < data.length; i++){
		...
		var nextQuarter = document.createElement("div");
		nextQuarter.setAttribute("class", data[i]["quarter_id"]+"-quarter");
		var nextQuarterTitle = document.createElement("div");
		nextQuarterTitle.setAttribute("class", "dropdown-title");
		nextQuarterTitle.innerHTML = data[i]["quarter_name"];
		nextQuarterTitle.addEventListener("click", showOrHideOptions);
		nextQuarter.append(nextQuarterTitle);
		var nextQuarterDropdown = document.createElement("div");
		nextQuarterDropdown.setAttribute("class", "dd-container dd-quarter-container hidden");
		nextQuarter.append(nextQuarterDropdown);
		productList.getElementsByClassName("dd-year-container")[0].append(nextQuarter);
	}
}
...
function initProductTab(data){
	...
	var productList = document.getElementsByClassName("dde-product-list")[0];
	for (var i = 0; i < data.length; i++){
		var parentQuarter = productList.getElementsByClassName(data[i]["quarter_id"]+"-quarter")[0];
		var nextProduct = document.createElement("div");
		...
		parentQuarter.getElementsByClassName("dd-quarter-container")[0].append(nextProduct);
	}
	loadingGraphic.hide();
}
