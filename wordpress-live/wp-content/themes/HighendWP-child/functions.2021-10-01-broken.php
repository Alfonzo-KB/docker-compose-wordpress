...
include_once("customProductStyles.php");
...
/*
 * Disable Payment Gateway for specific User Roles
 */
add_filter( 'woocommerce_available_payment_gateways', 'invoice_disable_manager' );
function invoice_disable_manager( $available_gateways ) {
	$user = wp_get_current_user();
	  if ( in_array( 'Wholesale20', $user->roles ) || in_array( 'Wholesale30', $user->roles ) ) {
	  	//User is allowed to use our invoice gateway
		}else{
			//User is not allowed to use our invoice gateway
...
add_action('woocommerce_single_product_summary',function(){
    ...
    $term_string = json_encode($terms);
    if(str_contains($term_string, "Digital Dealer Exclusives Shops")){
        the_field("subtitle");
    }
}, 5);
...
function is_wholesale_customer(){
	...
	$role_list = array("wholesale_customer", "Wholesale30", "Wholesale20", "Wholesale15", "administrator", "um_customer-webstore");
