		...
		<div class="container">
			...
			<?php 
				...
				if ( $sidebar_layout == "default" ) {
					$sidebar_layout = highend_option('hb_staff_layout'); 
					if ( $sidebar_layout != 'fullwidth')
						$sidebar_position = highend_option('hb_staff_sidebar_side');
			...
			<?php } ?>
					...
					<?php 
						...
						$image = $thumb ? highend_resize( $thumb, 250, 250 ) : false;
						...
						<?php 
							if ( $sidebar_layout == 'wpsidebar' ) {
								...
								foreach ($social_links as $soc_id => $soc_name) {
									...
									?>
										<?php if ($soc_id != 'behance' && $soc_id != 'vk' && $soc_id != 'envelop' && $soc_id != 'sn500px' && $soc_id != "weibo" && $soc_id != "tripadvisor" ) { ?>
				...
				<?php if ( highend_option('hb_staff_enable_related_posts') ) { get_template_part('includes/team','related-items'); } ?>
				</div>
				<!-- END .hb-main-content -->
